...
def preprocess_vis_image(**vizParams):
    visGen = random_visual_effect_generator(**vizParams)

    def _preprocess_image(image):
        image = (image/255.).astype(np.float32)
        visTransform = next(visGen)
        image = visTransform(image)

        return image

    return _preprocess_image

def rescale_images(**params):
    '''
    absord all parameters to be compatible with the func preprocess_vis_image
    :param params: *Not used*
    :return:
    '''
    def _rescale_images(image):
        image = (image/255.).astype(np.float32)
        return image

    return _rescale_images